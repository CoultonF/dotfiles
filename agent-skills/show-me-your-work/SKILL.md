...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript when the platform exposes one; otherwise walk the log against its files, commits, artifacts, traces, screenshots, and command outputs:
...
Before handing back, spawn one readonly subagent through OMP `task` or Zed `spawn_agent`. Prefer a different model family when selection is available; otherwise use a fresh subagent on the current model. It reads the audit trail, available transcript, changed files, and evidence, then flags what the user should pay attention to.
...
Every reply for a run that produced a trail ends with an "Attention" section. Identify the reviewer by model when known, otherwise by agent label, then list each flag pointing to specific rows or moments. "No flags" is a valid value.
