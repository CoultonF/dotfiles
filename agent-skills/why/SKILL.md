...
Before spawning investigators, list the MCP and built-in tools available in the current OMP or Zed session. Use their names, descriptions, and resource metadata to identify evidence systems.
...
Use OMP `task` or Zed `spawn_agent` for each investigator. Use a fast model when selectable; otherwise inherit the current model. Investigators must not write, even when MCP access requires an agent profile that technically permits writes.
...
Spawn one synthesizer through OMP `task` or Zed `spawn_agent`. Use the strongest reasoning model when selectable; otherwise inherit the current model. It must not write, even when citation spot-checks require MCP access.
